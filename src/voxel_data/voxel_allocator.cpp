...
Ptr allocNode() {
    if (freeListPop--) {
        ...
        blocks[node >> (BLOCK_SIZE_BITS-4)].modified = true;
    ...
    Ptr ptr = convertToPtr(nextAllocIndex >> 4);
    nextAllocIndex += 16;
    return ptr;
}
...
Ptr allocConsecNodes(int n) {
    ...
    if ((nextAllocIndex & BLOCK_SIZE_MASK) + (n << 4) > BLOCK_SIZE) {

        freeConsecNodes(nextAllocIndex >> 4, n);
        nextAllocIndex += n << 4;
